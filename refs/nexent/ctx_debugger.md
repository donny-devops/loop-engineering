...
# Source: https://github.com/ModelEngine-Group/nexent/blob/main/
# Reference only — not active in loop-engineering.
...
Commands below assume you're in this directory (README's location ctx_debugger/). Relative path conventions: . = ctx_debugger/, .. = sdk/, ../.. = nexent repo root directory (where sdk/, backend/, .env reside).
...
| `/history` | Accumulated session raw ledger (each turn verbatim, pre-compression) |
| `/summary` | Current compression summary full text |
| `/compress` | Last turn's compression LLM input prompt (fed in) and output summary (produced) |
...
| `llm [--tag main|compression]` | LLM call events |
...
Map trace into self-hosted Langfuse, get nested traces, per-call drill-down, token/duration views, session grouping.
...
Depends on langfuse SDK (`uv pip install langfuse`). Self-hosted Langfuse can be started with official docker compose.

Known limitation: Observations created at export time, single duration faithful, but absolute position on Langfuse timeline is export time, not original wall-clock time.
...
Text fields all bounded truncation (head N chars + `...[N chars elided]...` + tail M chars), avoid trace file infinite growth.

## 7. Design Principles

- Zero SDK source modification: Via monkey-patch wrapping + proxy objects, no changes to nexent/.
- Read-only public interface + few stable internal interfaces: Underscore interfaces like `_step_local_log`, `_effective_*_tokens` are also used by benchmark, treated as de-facto stable.
- Five optional layers: `layers` parameter narrows as needed, trace size controllable.
- Failure isolation: Each attach point try/except兜底, single layer failure only emits debug_error event, won't crash agent.
- Reuse Nexent's own events: observer layer directly mirrors MessageObserver, no reinventing wheel.
- No frontend pollution: Observer tap modifies instance's add_message, original method still called, frontend stream unaffected.

## 8. Coupling Points with Nexent
...
- Main LLM calls default only store digest: Compression LLM calls' input messages and output already stored verbatim in full. Main LLM calls default still truncated digest; pass `capture_full_messages=True` for full text. Interactive REPL already defaults this option on, so `/context` can see full text.
